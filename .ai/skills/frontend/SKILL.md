---
name: frontend
description: Next.js, React, TypeScript, Tailwind CSS, and Shadcn UI frontend development conventions.
triggers:
  - frontend UI development
  - React component creation
  - page routing and layout changes
  - form implementation
  - state management
...
- UI layout, styling, or responsive design
- Form implementation with validation
- State management (Zustand, React Query)
- Animation or motion design
- Client-side routing or middleware
...
1. Read the backend API gateway base URL from `NEXT_PUBLIC_API_URL` env var.
2. Configure the HTTP/Axios base client in `lib/api-client.ts`.
3. Never hardcode backend server URLs directly in the API service files.
...
- Never hardcode text — use constants or i18n keys.
- Never skip accessibility — all interactive elements need ARIA labels.
- Never use `useEffect` for data fetching — use React Query.
- Never hardcode API base URLs — use `NEXT_PUBLIC_API_URL`.

## Verification Expectations

- All pages render without errors
- Responsive design works at all breakpoints (320px–1536px)
